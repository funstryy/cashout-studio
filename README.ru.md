...
![Микшер в доке под аранжировкой](docs/screenshots/09-mixer.png)

Живёт в доке по нижнему краю рабочего пространства, на **F9**, деля его с
рэком каналов: по одному за раз, потому что на половине высоты не читается
ни то, ни другое. По линейке на дорожку плюс мастер, у каждой фейдер,
mute, solo, сегментный индикатор и показание усиления.

При выборе линейки справа показываются её **вставки**. У мастера своих
нет: он просчитывается в рамках экспорта, а не отпечатывается, так что
класть туда нечего.

Экспорт в шапке. **WAV или MP3**, просчёт офлайн через тот же граф
обработки, что и живое воспроизведение, а результат попадает обратно в
библиотеку новым треком, чтобы его можно было отмастерить, разделить или
положить на другой таймлайн, не выходя из студии.

## Встроенный DAW

![Рабочее пространство: браузер, аранжировка, док микшера и колонка инструментов](docs/screenshots/02-daw.png)

Рабочее пространство, а не страница. Строка меню, кнопки окон и транспорт
закреплены и не прокручиваются; аранжировка забирает всю оставшуюся
высоту; рэк и микшер делят изменяемый по высоте док внизу; ИИ-инструменты
стоят в колонке справа. Прокручивается только содержимое зон, поэтому
транспорт всегда там, где вы его оставили.

**Окна на F5–F9**, на тех же клавишах, что и в FL Studio: плейлист, рэк
каналов, пианоролл, браузер, микшер. Это переключатели: повторное нажатие
подсвеченной кнопки закрывает окно. Пианоролл живёт внутри рэка, поэтому
F7 открывает оба.

**Заголовок дорожки высотой в одну строку.** Имя, mute, solo, уровень,
панорама и индикатор: то, к чему тянутся во время воспроизведения. Полная
канальная линейка с эквалайзером, компрессией и ревербом, вставки
плагинов, замена вокала, список уже отпечатанного и удаление спрятаны за
кнопкой `⋯` в заголовке.

- Добавление аудио из библиотеки, файлового браузера или перетаскиванием
  файла прямо на дорожку.
- Неразрушающее перемещение и обрезка клипов с прилипанием к соседям и к
  нулю. `S` режет то, что под курсором воспроизведения.
- Отмена и повтор на 30 шагов, Ctrl+Z и Ctrl+Y.
- Автоматические рампы 15 мс на краях клипов, чтобы резкий стык не щёлкал.
- Защита от ухода со страницы с несохранённой работой.
- **Плагины VST3** работают вживую в тракте, когда устройством владеет
  нативный движок, и отпечатываются на дорожку на браузерном тракте, потому
  что нативный код внутри страницы работать не может.
...
- **Указание авторства**: если вы форкаете, копируете или используете код Cashout Studio в своих проектах, сохраняйте ссылку на этот репозиторий. Лицензия MIT выше уже требует сохранять копирайт-нотис в любой копии: это просто явное напоминание об этом требовании.
