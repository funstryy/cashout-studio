...
- **Mutually-exclusive orchestrator**: pick a model in the header; it starts up and the other stops on its own. No need to manually kill processes before starting the other engine.
...
![The mixer, docked under the arrangement](docs/screenshots/09-mixer.png)

Lives in the dock along the bottom of the workspace, on **F9**, sharing it
with the channel rack: one at a time, because neither is readable at half
height. A strip per track plus the master, each with a fader, mute, solo,
a segmented meter and a gain readout.

Selecting a strip shows its **inserts** on the right. The master has none
of its own: it is rendered as part of the export rather than printed, so
there is nothing to put in a slot there.

Export sits in the caption. **WAV or MP3**, rendered offline through the
same processing graph as live playback, and the result lands back in your
library as a new track, so it can be mastered, separated or dropped onto
another timeline without leaving the studio.

## Built-in DAW

![The workspace: browser, arrangement, mixer dock and tools rail](docs/screenshots/02-daw.png)

A workspace rather than a page. The menu bar, the window buttons and the
transport are pinned and never scroll; the arrangement takes whatever
height is left; the rack and mixer share a resizable dock along the
bottom; the AI tools sit in a rail on the right. Only the insides of those
zones scroll, so the transport is always where you left it.

**The windows are on F5 to F9**, the same keys FL Studio uses: playlist,
channel rack, piano roll, browser, mixer. They are toggles, so pressing
the lit one again closes it. The piano roll lives inside the rack, so F7
opens both.

**Track headers are one row high.** Name, mute, solo, level, pan and a
meter: the things you touch while the transport is rolling. The full
channel strip with EQ, compression and reverb, plugin inserts, re-voice,
the list of what has already been printed onto the track, and delete are
all behind the header's `⋯` button.

- Add audio from the library, the file browser, or by dragging a file
  straight onto a track.
- Non-destructive clip moving and edge trimming, with snapping to
  neighbouring clips and to zero. `S` splits whatever is under the
  playhead.
- Undo and redo across 30 steps, on Ctrl+Z and Ctrl+Y.
- Automatic 15 ms ramps at clip edges, so a hard cut does not click.
- Guards against navigating away from unsaved work.
- **VST3 plugins** run live in the signal path when the native engine owns
  the device, and are printed onto the track when playback is on the
  browser path, because native code cannot run inside the page.
